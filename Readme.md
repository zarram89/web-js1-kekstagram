...
<details>
<summary>module9-task2: Редактирование масштаба и применение эффектов</summary>
...
1. **Масштабирование**:
  - Реализовать логику изменения масштаба (25-100% с шагом 25%).
  - Обновлять значение в поле `.scale__control--value`.
  - Применять стиль `transform: scale()` к изображению.

2. **Эффекты**:
  - Подключить библиотеку noUiSlider.
  - Инициализировать слайдер для управления интенсивностью эффекта.
  - Реализовать применение CSS-фильтров (grayscale, sepia, invert, blur, brightness) в зависимости от выбранного эффекта и положения слайдера.
  - Обеспечить сброс слайдера и эффекта при переключении фильтров.
  - Скрывать слайдер для эффекта "Оригинал".

3. **Интеграция**:
  - Сбрасывать масштаб и эффекты при закрытии формы.

### Ход выполнения

1. **index.html**: Подключены стили и скрипт библиотеки noUiSlider.
2. **js/scale.js**: Создан модуль для управления масштабом. Реализованы обработчики кнопок "+" и "-", обновление стилей и сброс масштаба.
3. **js/effect.js**: Создан модуль для работы с эффектами. Настроен слайдер noUiSlider, реализована логика переключения эффектов и изменения их интенсивности.
4. **js/form.js**: Добавлен вызов функций сброса масштаба и эффектов при закрытии формы.
5. **js/main.js**: Импортированы и подключены новые модули.

### Проверка результата

#### Автоматизированное тестирование (Browser Subagent)
Был проведен сценарий тестирования в браузере:
1. Открытие формы загрузки.
2. Изменение масштаба: проверка уменьшения до 25% и увеличения до 100%.
3. Применение эффектов: переключение между Chrome, Sepia, Marvin, Phobos, Heat.
4. Проверка работы слайдера: изменение интенсивности эффектов.
5. Сброс: закрытие и повторное открытие формы вернуло масштаб 100% и эффект "Оригинал".

В ходе тестирования подтверждено корректное поведение всех компонентов.
...
<details>
<summary>module11-task1: Взаимодействие с сервером</summary>

### Задание

Бэкенд-разработчики сообщили, что сервер готов. Пора посмотреть, как происходит взаимодействие с настоящим сервером.
...
1.  **API Модуль (`js/api.js`)**:
  - Реализовать функцию `getData` (GET запрос) для загрузки фото.
  - Реализовать функцию `sendData` (POST запрос) для отправки формы.
  - Использовать адреса из ТЗ: `https://32.javascript.htmlacademy.pro/kekstagram/data` и `https://32.javascript.htmlacademy.pro/kekstagram`.

2.  **Сообщения (`js/message.js`)**:
  - Реализовать функции показа сообщений об успехе (`#success`) и ошибке (`#error`).
  - Реализовать `showAlert` для ошибок загрузки данных.
  - Обработать закрытие сообщений по Esc, клику вне области и клику на кнопку.

3.  **Интеграция (`js/main.js`)**:
  - Удалить моковые данные (`data.js`).
  - Загружать данные через `getData`.
  - Отрисовывать фото или показывать ошибку.

4.  **Форма (`js/validation.js`, `js/form.js`)**:
  - Перехватывать `submit` формы.
  - Блокировать кнопку отправки ("Сохраняю...").
  - Отправлять данные через `sendData`.
  - При успехе: закрывать форму, показывать Success.
  - При ошибке: показывать Error, не закрывать форму.
  - Доработать обработку Esc: не закрывать форму, если открыто сообщение об ошибке.

### Ход выполнения

1.  Создан файл `js/api.js` с методами `getData` и `sendData` на основе `fetch`.
2.  Создан файл `js/message.js` для управления DOM-элементами сообщений (клонирование шаблонов, обработчики событий).
3.  Модифицирован `js/main.js`: удален импорт `data.js`, добавлен вызов `getData`.
4.  Модифицирован `js/form.js`: экспортирована функция `closeUploadForm`, добавлен чек на наличие `.error` в `onDocumentKeydown`.
5.  Модифицирован `js/validation.js`: реализована отправка `FormData` через `sendData` с блокировкой кнопки.
6.  Удален файл `js/data.js`.

### Проверка результата

1.  **Загрузка данных**: При открытии страницы фотографии загружаются с сервера. При ошибке сети показывается алерт.
2.  **Отправка формы**:
  - При валидной форме кнопка блокируется.
  - Успешная отправка -> форма закрывается, появляется зеленое сообщение.
  - Неуспешная отправка -> появляется красное сообщение, формы остается открытой.
3.  **Интерфейс**: Esc закрывает сообщения, но не форму (если открыто сообщение об ошибке).
...
С сервера нам приходит достаточно большой список фотографий. В нашем случае мы каждый раз получаем по 25 фотографий. В реальном проекте их может быть значительно больше. Возникает вопрос: «А как, например, посмотреть только новые или самые обсуждаемые фотографии?» Решить эту проблему нам помогут фильтры.
...
### Задачи
1.  Реализовать функцию устранения дребезга (`debounce`).
2.  Модифицировать функцию отрисовки галереи, чтобы она очищала контейнер перед добавлением новых элементов.
3.  Создать модуль фильтрации:
  -   Показывать блок фильтров после загрузки данных.
  -   Обрабатывать клики по кнопкам фильтров.
  -   Реализовать алгоритмы фильтрации (случайные, обсуждаемые).
  -   Применять `debounce` при отрисовке.

### План выполнения
1.  **Утилиты (`js/util.js`)**: Добавить `debounce`.
2.  **Галерея (`js/gallery.js`)**: Добавить очистку `.picture` в `renderPictures`.
3.  **Фильтр (`js/filter.js`)**: Реализовать логику фильтрации и инициализацию.
4.  **Integration (`js/main.js`)**: Подключить фильтры после загрузки данных.

### Ход выполнения
(Будет заполнено по мере реализации)

### Проверка результата
Проверять будем переключением фильтров и визуальным контролем количества и порядка фотографий, а также отсутствием "дребезга".
...
### Задачи
1.  Реализовать чтение файла из инпута `.img-upload__input`.
2.  Подставить прочитанное изображение в `.img-upload__preview img`.
3.  Подставить изображение в миниатюры эффектов `.effects__preview`.

### План выполнения
1.  **form.js**: В обработчик открытия формы добавить чтение файла и обновление DOM-элементов.

### Ход выполнения
(Будет заполнено по мере реализации)

### Проверка результата
Загрузка различных изображений (разные форматы), проверка отображения.
