...
# План реализации: Взаимодействие с сервером (module11-task1)

## Описание задачи
Заменить моковые данные на реальные с сервера и реализовать отправку формы через AJAX.

## Адреса API
- **GET**: `https://32.javascript.htmlacademy.pro/kekstagram/data` - получение фотографий
- **POST**: `https://32.javascript.htmlacademy.pro/kekstagram` - отправка формы

## Предложенные изменения

### 1. Новые модули

#### [NEW] [js/api.js](file:///d:/antigravity/anti-keksagram/js/api.js)
Модуль для взаимодействия с сервером.
- `getData(onSuccess, onFail)`: получение данных
- `sendData(onSuccess, onFail, body)`: отправка данных (FormData)

#### [NEW] [js/message.js](file:///d:/antigravity/anti-keksagram/js/message.js)
Модуль для показа сообщений.
- `showSuccessMessage()`: показ шаблона `#success`
- `showErrorMessage()`: показ шаблона `#error`
- `showAlert()`: показ шаблона `#data-error` (или простого alert, см. ТЗ п. 4.2)
- Обработка закрытия по Esc, клику вне блока, клику на кнопку.
- Особенность: при показе ошибки `#error` закрытие сообщения не должно закрывать форму.

### 2. Доработка существующих модулей

#### [MODIFY] [js/main.js](file:///d:/antigravity/anti-keksagram/js/main.js)
- Удалить генерацию моковых данных (`data.js`).
- Вызвать `getData`.
- В `onSuccess` вызвать `renderPictures`.
- В `onFail` показать сообщение об ошибке загрузки (шаблон `#data-error` исчезает через 5 сек).

#### [MODIFY] [js/validation.js](file:///d:/antigravity/anti-keksagram/js/validation.js)
- В обработчике `submit`:
  - Блокировать кнопку отправки.
  - Вызвать `sendData` с `new FormData(evt.target)`.
  - В `onSuccess`:
    - Закрыть форму (`closeUploadForm`).
    - Показать сообщение успеха (`showSuccessMessage`).
  - В `onFail`:
    - Показать сообщение ошибки (`showErrorMessage`).
    - **Важно**: форму не закрывать.
  - В `finally`: разблокировать кнопку.

#### [MODIFY] [js/form.js](file:///d:/antigravity/anti-keksagram/js/form.js)
- Убедиться, что `closeUploadForm` сбрасывает все поля (уже реализовано).
- Добавить экспорт функции сброса/закрытия, если нужно вызывать из validation.
...
### Получение данных
1. Открыть страницу -> запрос на `/data`.
2. Успех -> отрисованы миниатюры.
3. Ошибка (отключить сеть) -> сообщение "Не удалось загрузить данные" (исчезает через 5 сек).

### Отправка формы
1. Заполнить форму корректно -> Отправить.
